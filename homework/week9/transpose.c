...
void transpose_blocking(int n, int blocksize, int *dst, int *src) {
    ...
    for (x = 0; x < n; x += blocksize) {
        for (y = 0; y < n; y += blocksize) {
            for (int block_x = x; block_x < x + blocksize && block_x < n; block_x++) {
                for (int block_y = y; block_y < y + blocksize && block_y < n; block_y++) {
                    dst[block_y + block_x * n] = src[block_x + block_y * n];
                }
            }
        }
    }
    // finish remaining items if blocksize is not a multiple of n
    for (; x < n; x++) {
        for (; y < n; y++) {
            dst[y + x * n] = src[x + y * n];
        }
